...
while True:   
    ...
    # for (x, y, w, h) in faces:
        ...
        if stable_emotion[-1] != stable_emotion[-2]:
            ...
            control_arduino(f"SET_EMOTION_{stable_emotion[-1].upper()}")
        # turn_led(stable_emotion[-1])
        ...
        if ear < EYE_AR_THRESH:
            ...
            if COUNTER >= EYE_AR_CONSEC_FRAMES:
                ...
                turn_led('Eyes')                
                # send signal to Arduino to close eyes
                control_arduino("CLOSE_EYES")
                if not headless:
        ...
        #        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,0,0),2)
            ...
            turn_led('Eyes', False)
            # send signal to Arduino to open eyes 
            control_arduino("OPEN_EYES")
            COUNTER = 0
            
        # Draw text if mouth is open
        if mar > MOUTH_AR_THRESH:
            turn_led('Mouth')
            # send signal to Arduino to open mouth 
            control_arduino("OPEN_MOUTH")
            if not headless:
                cv2.putText(frame, "Mouth is Open!", (x+w,y+100),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,0,255),2)
        else:
            turn_led('Mouth', False)
            # send signal to Arduino to close mouth 
            control_arduino("CLOSE_MOUTH")
